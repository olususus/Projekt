...
#                         else:
#                             messagebox.showwarning("Ostrzeżenie", "Brak danych pogodowych dla podanego miasta.")
#                     else:
#                         messagebox.showwarning("Ostrzeżenie", "Brak danych pogodowych dla podanego miasta.")
#                 else:
#                     messagebox.showwarning("Ostrzeżenie", "Nie znaleziono podanego miasta.")
#             except Exception as e:
#                 messagebox.showerror("Błąd", f"Wystąpił błąd: {str(e)}")
#         else:
#             messagebox.showerror("Błąd", "Proszę wprowadzić nazwę miasta.")
class StartScreen:
    def __init__(self, master):
        self.master = master
...
class HotelApp(ctk.CTk):
    def __init__(self):
        ...
        self.label_email = ctk.CTkLabel(text="Adres email klienta:")
        self.label_email.grid(row=1, column=0)

        self.label_start_date = ctk.CTkLabel(text="Data rozpoczęcia (RRRR-MM-DD):")
        self.label_start_date.grid(row=2, column=0)

        self.label_end_date = ctk.CTkLabel(text="Data zakończenia (RRRR-MM-DD):")
        ...
        self.submit_button = ctk.CTkButton(text="Zarezerwuj", command=self.submit_reservation)
        self.submit_button.grid(row=4, column=0, columnspan=2)

        self.additional_features_button = ctk.CTkButton(text="Dodatkowe Funkcje", command=self.open_additional_features())
        self.additional_features_button.grid(row=5, column=0, columnspan=2)

        self.button_explore = ctk.CTkButton(text="Zwiedzanie", command=self.explore_places)
        self.button_explore.grid(row=6, column=0, columnspan=2)


    def explore_places(self):
        location = simpledialog.askstring("Zwiedzanie", "Podaj lokalizację:")
        if location:
            try:
                ...
                if "results" in data:
                    ...
                    messagebox.showwarning("Ostrzeżenie", "Brak danych o popularnych miejscach.")
            except Exception as e:
                messagebox.showerror("Błąd", f"Wystąpił błąd: {str(e)}")
        else:
            messagebox.showerror("Błąd", "Proszę wprowadzić lokalizację.")
            ...
            try:
                ...
                if customer_id is not None:
                    ...
                    if self.db.add_reservation(reservation):
                        messagebox.showinfo("Sukces", "Rezerwacja została dodana.")
                    else:
                        messagebox.showerror("Błąd", "Wystąpił błąd podczas dodawania rezerwacji.")
                else:
                    messagebox.showerror("Błąd", "Wystąpił błąd podczas dodawania klienta.")
    ...
    def find_directionsnotworking1(self):
        location = simpledialog.askstring("Znajdź dojazd", "Podaj miejsce, do którego chcesz dojechać:")
        if location:
            try:
            ...
            messagebox.showerror("Błąd", "Proszę wprowadzić miejsce docelowe.")
    ...
    #         messagebox.showerror("Błąd", "Wybierz dokładnie dwa przystanki!")
    #         return
    ...
    #         messagebox.showinfo("Informacja", "Brak wspólnych tras między wybranymi przystankami.")
    #     else:
    #         messagebox.showinfo("Informacja", f"Wspólne trasy między wybranymi przystankami: {', '.join(common_routes)}")

    
        # Tworzenie interaktywnej mapy Google Maps
    
    def find_nearby_restaurants(self):
        location = simpledialog.askstring("Znajdź Restauracje", "Podaj lokalizację (miasto, ulica, itp.):")
        if location:
            try:
                ...
                if "results" in data:
                    ...
                    if restaurants:
                        ...
                        messagebox.showinfo("Restauracje w Okolicy", "Brak restauracji w pobliżu.")
                else:
                    messagebox.showerror("Błąd", "Brak danych o restauracjach.")
            except Exception as e:
                messagebox.showerror("Błąd", f"Wystąpił błąd: {str(e)}")
        else:
            messagebox.showerror("Błąd", "Proszę wprowadzić lokalizację.")
    ...
    def admin_mode(self):
        password = simpledialog.askstring("Admin Mode", "Podaj hasło:")
        if password == "123123":
    ...
    def playground(self):
        ...
        messagebox.showinfo("Bawialnia", "W Bawialni dla dzieci: 'Dzieci w Kulkach™' znajdują się:\n- Zabawki\n- Kulki\n- Telewizor z bajkami\n- Konsola XBOX oraz PLAYSTATION\n- Duży wybór płyt CD.")

    def pool(self):
        # Tutaj możesz dodać funkcje związane z brodzikiem dla dzieci
        messagebox.showinfo("Brodzik", "W Brodziku: 'Woda z Dziecmi™; znajduje się:\n- Woda w temperaturze pokojowej\n- Zjeżdzalnia\n- Zabawki wodne\n- Pistolety wodne.")


    def delete_reservation(self):
        reservation_id = simpledialog.askinteger("Usuń rezerwację", "Podaj ID rezerwacji do usunięcia:")
        if reservation_id:
            if self.db.delete_reservation(reservation_id):
                messagebox.showinfo("Sukces", "Rezerwacja została usunięta.")
            else:
                messagebox.showerror("Błąd", "Błąd podczas usuwania rezerwacji.")
        else:
            messagebox.showerror("Błąd", "Nieprawidłowe ID rezerwacji.")
    ...
    def export_data(self):
        ...
        if file_name:
            ...
            messagebox.showinfo("Sukces", "Dane zostały pomyślnie wyeksportowane do pliku.")
